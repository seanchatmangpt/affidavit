...
fn measure_pipeline_throughput(n: usize) -> (f64, std::time::Duration) {
    ...
    for i in 0..n {
        let event = build_event(
            ...
            vec![object_ref(black_box(format!("obj-{i}")), black_box("artifact"))],
            black_box(format!("payload-{i}").as_bytes()),
            &mut counter,
        ).expect("failed to build event during throughput test");
        
        asm.append(event).expect("failed to append event during throughput test");
    }
    ...
    // 4. Validate success
    assert!(verdict.accepted, "Throughput test receipt was rejected: {}", verdict.reason);
...
fn test_emit_scaling_harness() {
    ...
    println!("  n={:>3} | throughput: {:>10.2} ops/sec", large_n, large_tps);
...
fn main() { test_throughput_harness_execution(); test_emit_scaling_harness(); println!("All throughput tests passed!"); }
