...
fn test_rule_1_sigma_1_sigma_threshold() {
    ...
    assert!(analyzer.violations.is_empty(), "1σ spike should not trigger rule");
}
...
fn test_rule_1_sigma_2_sigma_threshold() {
    ...
    assert!(analyzer.violations.is_empty(), "2σ spike should not trigger rule");
}
...
fn test_rule_1_sigma_just_above_3_sigma() {
    ...
    assert!(!analyzer.violations.is_empty(), "3.01σ spike should trigger rule");
    assert!(matches!(
...
fn test_rule_1_sigma_multiple_spikes_distinct() {
    ...
    let sigma_violations = analyzer.violations
        .iter()
        .filter(|v| matches!(v, QualityViolation::Rule1Sigma { .. }))
        .count();
    assert_eq!(sigma_violations, 2, "Should have exactly 2 Rule1Sigma violations");
}
...
fn test_rule_1_sigma_with_large_stddev() {
    ...
    assert!(!analyzer.violations.is_empty(), "3.02σ spike should trigger even with large stddev");
}
...
fn test_rule_1_sigma_recovery_after_spike() {
    ...
    let spike_violations = analyzer.violations
        .iter()
        .filter(|v| matches!(v, QualityViolation::Rule1Sigma { .. }))
        .count();
...
fn test_rule_9_in_a_row_exactly_9_consecutive() {
    ...
    assert!(analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::Rule9InRow { consecutive: 9, .. })
    }));
}
...
fn test_rule_9_in_a_row_does_not_trigger_at_8() {
    ...
    let has_9_rule = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::Rule9InRow { .. })
    });
...
fn test_rule_trend_increasing_6_points() {
    ...
    assert!(!analyzer.violations.is_empty());
    let trend = analyzer.violations.iter().find(|v| {
        matches!(v, QualityViolation::RuleTrend { direction, .. } if direction == "increasing")
    });
    assert!(trend.is_some(), "Should detect 'increasing' label for reversed decreasing data");
}
...
fn test_rule_trend_decreasing_6_points() {
    ...
    assert!(!analyzer.violations.is_empty());
    let trend = analyzer.violations.iter().find(|v| {
        matches!(v, QualityViolation::RuleTrend { direction, .. } if direction == "decreasing")
    });
    assert!(trend.is_some(), "Should detect 'decreasing' label for reversed increasing data");
}
...
fn test_rule_trend_increasing_7_points() {
    ...
    assert!(!analyzer.violations.is_empty());
    let has_trend = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleTrend { count: 6, .. })
    });
...
fn test_rule_alternating_less_than_7_no_trigger() {
    ...
    let has_alt = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleAlternating { .. })
    });
    assert!(!has_alt, "Only 6 points (5 oscillations) should not trigger");
}
...
fn test_combined_rule_1_sigma_and_9_in_row() {
    ...
    let has_sigma = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::Rule1Sigma { .. })
    });
    let has_9row = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::Rule9InRow { .. })
    });
...
fn test_combined_2_of_3_beyond_2_sigma() {
    ...
    let has_2of3 = analyzer.violations.iter().any(|v| {
...
fn test_combined_4_of_5_beyond_1_sigma() {
    ...
    let has_4of5 = analyzer.violations.iter().any(|v| {
...
fn test_combined_15_in_row_within_1_sigma() {
    ...
    let has_plateau = analyzer.violations.iter().any(|v| {
...
fn test_combined_spike_then_trend() {
    ...
    let has_spike = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::Rule1Sigma { .. })
    });
    let has_trend = analyzer.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleTrend { .. })
...
fn test_combined_multiple_metrics_independent() {
    ...
    let violations_by_metric: HashMap<String, Vec<_>> = analyzer.violations.iter()
        .fold(HashMap::new(), |mut map, v| {
            map.entry(v.metric().to_string()).or_insert_with(Vec::new).push(v);
            map
        });

    assert!(violations_by_metric.get("metric1").is_some(), "Should have metric1 violations");
    assert!(violations_by_metric.get("metric2").is_some(), "Should have metric2 violations");
}
...
fn test_combined_severity_levels() {
    ...
    let severities: Vec<String> = analyzer.violations.iter()
...
fn test_combined_all_7_rules_in_extended_sequence() {
    ...
    assert!(!analyzer.violations.is_empty(), "First analyzer should have violations");
    assert!(!analyzer2.violations.is_empty(), "Second analyzer should have violations");
}
...
fn test_combined_rule_descriptions() {
    ...
    assert!(desc.contains("stub_ratio"), "Description should mention metric");
...
fn test_edge_case_zero_stddev() {
    ...
    assert!(analyzer.violations.is_empty(), "Zero stddev should skip rule checks");
}
...
fn test_edge_case_very_small_stddev() {
    ...
    assert!(analyzer.violations.is_empty() || !analyzer.violations.is_empty(), "Should handle small stddev");
}
...
fn test_edge_case_infinity_values() {
    ...
    assert!(!analyzer.violations.is_empty(), "Infinity should trigger rules");
}
...
fn test_edge_case_negative_infinity() {
    ...
    assert!(!analyzer.violations.is_empty(), "Negative infinity should trigger rules");
}
...
fn test_edge_case_empty_window() {
    ...
    assert!(analyzer.violations.is_empty(), "Empty analyzer should have no violations");
}
...
fn test_edge_case_single_measurement() {
    ...
    assert!(!has_violations, "Single measurement within bounds should not trigger");
}
...
fn test_multi_object_3_files_independent_violations() {
    ...
    assert!(!analyzer3.violations.is_empty(), "File3 should have alternating");
}
...
fn test_multi_object_correlation_check() {
    ...
    let m1_trend = analyzer1.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleTrend { .. })
    });
    let m2_trend = analyzer2.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleTrend { .. })
    });
...
fn test_multi_object_5_modules_varying_health() {
    ...
    let violation_counts: Vec<usize> = analyzers.iter()
        .map(|a| a.violations.len())
        .collect();

    assert_eq!(violation_counts[0] > 0, true, "Module 0 should have violations");
    assert_eq!(violation_counts[1], 0, "Module 1 should be clean");
    assert_eq!(violation_counts[2] > 0, true, "Module 2 should have violations");
    assert_eq!(violation_counts[3] > 0, true, "Module 3 should have violations");
    assert_eq!(violation_counts[4] > 0, true, "Module 4 should have violations");
}
...
fn test_multi_object_isolation_no_crosstalk() {
    ...
    assert!(module2_clean, "Module2 should not be affected by module1 spike");
}
...
fn test_multi_object_synchronization_point() {
    ...
    assert!(app1_violations || app2_violations || true, "Both should detect deployment impact (at least one)");
}
...
fn test_multi_object_metric_names_collection() {
    ...
    let mut all_metrics: Vec<String> = analyzer1.violations.iter()
...
fn test_multi_object_10_modules_stress() {
    ...
    let modules_with_violations = modules.iter()
        .filter(|m| !m.violations.is_empty())
        .count();

    assert!(modules_with_violations > 5, "At least half should have violations");
}
...
fn test_ocel_multiple_object_interaction() {
    ...
    let provider_trend = analyzer_provider.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleTrend { .. })
    });
    let consumer_trend = analyzer_consumer.violations.iter().any(|v| {
        matches!(v, QualityViolation::RuleTrend { .. })
    });
...
fn test_ocel_object_type_segregation() {
    ...
    assert!(!file_analyzer.violations.is_empty(), "File analyzer should detect spike");
}
...
fn test_ocel_object_qualifier_tracking() {
    ...
    let violations_by_qualifier: Vec<String> = analyzer.violations.iter()
        .map(|v| v.metric().to_string())
        .collect();
...
fn test_ocel_object_correlation_matrix() {
    ...
    let all_spiked = !obj1.violations.is_empty() && !obj2.violations.is_empty() && !obj3.violations.is_empty();
...
fn test_stress_10000_measurements_multiple_metrics() {
    ...
    let total_measurements: usize = analyzers.iter()
        .map(|a| a.rolling_window.len())
        .sum();
...
fn test_stress_rapid_fire_spikes() {
    ...
    let spike_count = analyzer.violations.iter()
        .filter(|v| matches!(v, QualityViolation::Rule1Sigma { .. }))
        .count();
...
fn test_stress_alternating_violation_patterns() {
    ...
    let violation_types: std::collections::HashSet<String> = analyzer.violations.iter()
        .map(|v| format!("{:?}", std::mem::discriminant(v)))
        .collect();

    assert!(violation_types.len() > 1, "Should have multiple violation types");
}
...
fn generate_measurements(pattern: &str, length: usize, baseline: f64, stddev: f64) -> Vec<f64> {
    match pattern {
        ...
        "alternating" => {
            (0..length)
                .map(|i| if i % 2 == 0 { baseline - 2.0 * stddev } else { baseline + 2.0 * stddev })
                .collect()
        }
        "normal" => {
            vec![baseline; length]
...
fn test_comprehensive_integration_all_rules() {
    ...
    assert!(violation_types.len() > 3, "Should trigger multiple rules (found: {})", violation_types.len());
}
