...
fn ocel_event_builder_carries_activity_time_and_attributes() {
    ...
    assert!(matches!(e.attributes()[0].value, OcelAttributeValue::Integer(42)));
}

#[test]
fn ocel_object_builder_carries_type_and_attributes() {
    let o = Object::new("o1", "order")
        .with_attribute(OcelAttribute::boolean("priority", true));

    assert_eq!(o.id(), "o1");
    assert_eq!(o.object_type(), "order");
    assert_eq!(o.attributes().len(), 1);
    assert!(matches!(o.attributes()[0].value, OcelAttributeValue::Boolean(true)));
