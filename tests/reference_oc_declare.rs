...
use wasm4pm_compat::declare::{
    Activity, DeclareConstraint, DeclareScope, DeclareTemplate, OcDeclareConstraint, OcDeclareRefusal,
};
...
fn oc_declare_refuses_empty_object_type_list() {
    let c = OcDeclareConstraint::new(base(DeclareScope::SingleObjectScope("order".into())), Vec::<String>::new());
...
fn oc_declare_refuses_synchronization_with_single_type() {
    ...
    let c = OcDeclareConstraint::synchronized(
        base(DeclareScope::SynchronizedObjectScope(vec!["order".into(), "item".into()])),
        vec!["order".into()], // only one type
    );
    assert_eq!(c.validate(), Err(OcDeclareRefusal::SynchronizationRequiresMultipleTypes));
}
...
fn oc_declare_admits_well_formed_constraint() {
    ...
    assert_eq!(c.validate(), Ok(()), "non-empty types, non-sync, single scope → admits");
