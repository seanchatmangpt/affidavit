...
fn process_cube_dimensions_are_constructed() {
    let all = [Dim::Activity, Dim::Resource, Dim::Time, Dim::DataAttribute, Dim::ObjectType, Dim::CaseAttribute];
    fn ok(d: Dim) -> bool {
        matches!(d, Dim::Activity | Dim::Resource | Dim::Time | Dim::DataAttribute | Dim::ObjectType | Dim::CaseAttribute)
    }
...
fn object_centricity_axis_is_constructed() {
    ...
    fn label(o: OC) -> &'static str {
        match o { OC::CaseCentric => "case", OC::ObjectCentric => "object", OC::Mixed => "mixed" }
    }
    let s: std::collections::BTreeSet<&str> = all.iter().copied().map(label).collect();
    assert_eq!(s.len(), 3, "three object-centricity modes (the OCEL convergence/divergence axis)");
}

#[test]
fn evidence_lifecycle_modes_are_constructed() {
    // The runtime mirror of the Evidence<_, State, _> typestate lifecycle.
    let all = [Mode::Raw, Mode::Parsed, Mode::Admitted, Mode::Refused, Mode::Projected, Mode::Exportable, Mode::Witnessed, Mode::Receipted];
    ...
    let families: std::collections::BTreeSet<&str> = all.iter().copied().map(stage_family).collect();
