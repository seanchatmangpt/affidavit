...
fn event_carries_all_perspectives() {
    let e = Event::new("place_order").at_ns(1_000).by("alice").with_lifecycle("complete");
...
fn well_formed_log_validates_and_counts_correctly() {
    ...
    let t2 = Trace::from_events([Event::new("create").at_ns(1), Event::new("release").at_ns(2)]);
    ...
    assert!(log.traces()[0].validate().is_ok(), "monotonic 3-event trace admits");
    assert!(log.traces()[1].validate().is_ok(), "monotonic 2-event trace admits");
}
